const PUBLIC_TOKENS = [
    'Bearer AAAAAAAAAAAAAAAAAAAAANRILgAAAAAAnNwIzUejRCOuH5E6I8xnZz4puTs%3D1Zv7ttfk8LF81IUq16cHjhLTvJu4FA33AGWWjCpTnA', // twitter for web
    'Bearer AAAAAAAAAAAAAAAAAAAAAFQODgEAAAAAVHTp76lzh3rFzcHbmHVvQxYYpTw%3DckAlMINMjmCwxUcaXbAN4XqJVdgMJaHqNOFgPMK0zN1qLqLQCF' // new tweetdeck (x pro)
];
...
const proxyRoutes = [
        ...
        beforeSendBody: (xhr, body) => {
            ...
            if(params.in_reply_to_status_id) {
                ...
                if(params.exclude_reply_user_ids) {
                    variables.reply.exclude_reply_user_ids = params.exclude_reply_user_ids.split(",");
                }
            }
            if(params.media_ids) {
    ...
    // Retweeting
    {
        ...
        beforeRequest: (xhr) => {
            ...
            xhr.storage.tweet_id = originalUrl.pathname.match(/\/1.1\/statuses\/retweet\/(\d+).json/)[1];
    ...
    // Unretweeting
    {
        ...
        beforeRequest: (xhr) => {
            let originalUrl = new URL(xhr.originalUrl);
            xhr.storage.tweet_id = originalUrl.pathname.match(/\/1.1\/statuses\/unretweet\/(\d+).json/)[1];
            xhr.storage.retweeter = getCurrentUserId();
        ...
        beforeSendBody: (xhr, body) => {
            return JSON.stringify({"variables":{"source_tweet_id":xhr.storage.tweet_id,"dark_request":false},"queryId":"iQtK4dl5hBmXewYZuEOKVw"})
        },
        afterRequest: xhr => {
            let data;
            try {
    ...
    // Getting tweet details
    {
        ...
        beforeRequest: xhr => {
            let originalUrl = new URL(xhr.originalUrl);
            xhr.storage.tweet_id = originalUrl.pathname.match(/\/1.1\/statuses\/show\/(\d+).json/)[1];
            xhr.modUrl = `https://twitter.com/i/api/graphql/KwGBbJZc6DBx8EKmyQSP7g/TweetDetail?variables=${encodeURIComponent(JSON.stringify({
        ...
        afterRequest: xhr => {
            ...
            let ic = data.data.threaded_conversation_with_injections_v2.instructions.find(i => i.type === "TimelineAddEntries").entries.find(e => e.entryId === `tweet-${xhr.storage.tweet_id}`).content.itemContent;
    ...
    // Tweet replies
    {
        ...
        afterRequest: xhr => {
            ...
            for(let id in data.globalObjects.tweets) {
                ...
                if(tweet.conversation_id_str) tweet.conversation_id = parseInt(tweet.conversation_id_str);
            ...
            if(entries) {
                ...
                for(let entry of entries.addEntries.entries) {
                    if(entry.entryId.startsWith('conversationThread-')) {
                        ...
                        if(entry.content.timelineModule.items) for(let item of entry.content.timelineModule.items) {
